    ...
    return bundle({
      ...
      .then(colorsLess => {
        const mappings = Object.assign(
          generateColorMap(colorsLess, customColorRegexArray),
          generateColorMap(mainLessFile, customColorRegexArray),
        );
      ...
      .then(([css, mappings, colorsLess]) => {
        ...
        // 优化 background-image 属性
        css = css.replace(/background-image: url\(\'[A-z0-9-_+]+\.[A-z]+\'\);/gi, '');
