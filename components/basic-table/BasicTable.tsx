...
import { Tooltip, Table, Input, Icon, Row, Col, Button, ConfigProvider } from 'antd';
...
function BasicTable<T>(props: IBasicTableProps<T>) {
  ...
  const showReloadBtn2FilterRight =
    (!showSearch || searchPos !== 'full') && showReloadBtn && reloadBtnPos === 'right';
  const showReloadBtn2FilterLeft = showReloadBtn && reloadBtnPos === 'left';

  const searchPlaceholder = props.searchPlaceholder || t('table.search.placeholder');
  ...
  const [columnsState, columnsDispatch] = useReducer(columnsReducer, columnsMap, columnInit);
      ...
      newDataSource = _.filter(newDataSource, (sourceItem) => {
        return Object.entries(queryFormValues).every(([queryKey, queryValue]) => {
          ...
          if (typeof queryValue === 'string') {
            return sourceItem[queryKey] && sourceItem[queryKey].indexOf(queryValue) >= 0;
          }
    ...
    return (
      <Tooltip placement="topLeft" title={searchPlaceholder}>
        <Input
          ...
          prefix={
            reloadBtnPos === 'right' && <Icon type="search" style={{ color: 'rgba(0,0,0,.25)' }} />
          }
          suffix={
            reloadBtnPos === 'left' && <Icon type="search" style={{ color: 'rgba(0,0,0,.25)' }} />
          }
          allowClear={true}
          value={searchQuery}
